...
sed -i "45c\$AMBERHOME/bin/MMPBSA.py -O -i mmpbsa.in -o ${pdbid}_FINAL_RESULTS_MMPBSA.dat -sp ${pdbid}_solv.prmtop -cp ${pdbid}_dry.prmtop -rp ${pdbid}_protein.prmtop -lp ${pdbid}_ligand.prmtop -y ${pdbid}_wat_min2.rst > ${pdbid}_progress.log" amber18-mmpbsa.bsub
sed -i "6c\#BSUB -J amber-${1}" amber18-mmpbsa.bsub
}
...
for i in $(ls *_solv.prmtop);do
	...
	safe=5
	while [ $safe -gt 0 ];do
		if [ -e amber18-mmpbsa.bsub ] && [ -e ${name}_solv.prmtop ] && [ -e ${name}_dry.prmtop ] && [ -e ${name}_ligand.prmtop ] && [ -e mmpbsa.in ];then
