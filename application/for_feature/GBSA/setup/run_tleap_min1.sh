# In minimization,tleap.in wat_min1.in wat_min2.in and amber18-gpu.bsub should be prepared
# "*_complex.pdb", "*_ligand.mol2_1", "*_ligand.frcmod" prepared from setup can generate *_solv.inpcrd, *_solv.prmtop
source ~/.cshrc > /dev/null 2>/dev/null
cd ../minimization
cat > tleap.in << EOF
...
sed -i "45c\$AMBERHOME/bin/pmemd.cuda -O -i wat_min1.in -o ${pdbid}_wat_min1.log -c ${pdbid}_solv.inpcrd -p ${pdbid}_solv.prmtop -r ${pdbid}_wat_min1.rst -ref ${pdbid}_solv.inpcrd" amber18-gpu.bsub
sed -i "6c\#BSUB -J AMber-${1}" amber18-gpu.bsub
}
